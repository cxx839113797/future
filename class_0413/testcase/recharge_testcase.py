...
class Login_Testcase(unittest.TestCase):
    case = do_excel(excel_data.excel_name, excel_data.recharge_sheet)
    cases=case.read()
    case.close()
    def setUp(self):
        self.session=http_requests()
        self.session.Session("POST","http://test.lemonban.com/futureloan/mvc/api/member/login",{"mobilephone":"18680073467" ,"pwd":"123456"})
    @data(*cases)
    def test_recharge(self,case):
        resp=self.session.Session(case.method,case.url,case.data)
        datafill=do_excel(excel_data.excel_name, excel_data.recharge_sheet)
